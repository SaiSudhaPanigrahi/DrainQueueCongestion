...
// BbrSenderOld implements BBR congestion control algorithm.  BBR aims to estimate
...
class BbrSenderOld : public SendAlgorithmInterface {
 public:
  enum Mode {
  ...
  // control issues.
  struct DebugState {
    explicit DebugState(const BbrSenderOld& sender);
  ...
  BbrSenderOld(ProtoTime now,const RttStats* rtt_stats,
            const UnackedPacketMap* unacked_packets,
            QuicPacketCount initial_tcp_congestion_window,
            QuicPacketCount max_tcp_congestion_window,
            Random* random);
  BbrSenderOld(const BbrSenderOld&) = delete;
  BbrSenderOld& operator=(const BbrSenderOld&) = delete;
  ~BbrSenderOld() override;
...
std::ostream& operator<<(std::ostream& os,
                                             const BbrSenderOld::Mode& mode);
std::ostream& operator<<(
    std::ostream& os,
    const BbrSenderOld::DebugState& state);
}
