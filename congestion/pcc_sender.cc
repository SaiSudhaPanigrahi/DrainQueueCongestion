...
using namespace ns3;
namespace dqc {
NS_LOG_COMPONENT_DEFINE("PCC_SENDER");
#else
namespace gfe_quic {
...
#ifdef QUIC_PORT
TimeDelta PccSender::ComputeMonitorDuration(
...
#else
ProtoTime PccSender::ComputeMonitorDuration(
    QuicBandwidth sending_rate,
    ProtoTime rtt) {
...
#ifdef QUIC_PORT
PccSender::PccSender(ProtoTime now,const RttStats* rtt_stats,
                     const UnackedPacketMap* unacked_packets,
                     QuicPacketCount initial_congestion_window,
                     QuicPacketCount max_congestion_window,
                     Random* random)
#else
PccSender::PccSender(ProtoTime initial_rtt_us,
                     QuicPacketCount initial_congestion_window,
                     QuicPacketCount max_congestion_window)
#endif
    : mode_(STARTING),
      ...
      sending_rate_(QuicBandwidth::FromBitsPerSecond(
          ...
          kNumMicrosPerSecond / rtt_stats->initial_rtt().ToMicroseconds())),
...
#endif
#if defined(QUIC_PORT) && defined(QUIC_PORT_LOCAL)
	NS_LOG_INFO(rtt_stats_->smoothed_rtt().ToMicroseconds());
...
void PccSender::OnCongestionEvent(bool rtt_updated,
  ...
  if (avg_rtt_us == 0) {
#ifdef QUIC_PORT
    DLOG_IF(FATAL,mode_ != STARTING);
    avg_rtt_us = rtt_stats_->initial_rtt().ToMicroseconds();
#endif
  } else {
#ifndef QUIC_PORT
    if (avg_rtt_ == 0) {
...
QuicByteCount PccSender::GetCongestionWindow() const {
  ...
  int64_t rtt_us = rtt_stats_->smoothed_rtt().ToMicroseconds() == 0
                       ? rtt_stats_->initial_rtt().ToMicroseconds()
                       : rtt_stats_->smoothed_rtt().ToMicroseconds();
...
string PccSender::GetDebugState() const {
  ...
  std::string msg = QuicStrCat(
      ...
      "pu=", ProtoStringPrintf("%.15g", latest_utility_info_.utility), ",",
