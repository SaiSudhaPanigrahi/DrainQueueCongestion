...
#ifdef QUIC_PORT
    DLOG(INFO) << "OnPacketSent called with empty queue.";
#endif
    return;
  }
...
void PccMonitorIntervalQueue::OnCongestionEvent(
  ...
  for (MonitorInterval& interval : monitor_intervals_) {
...
#ifdef QUIC_PORT
                            TimeDelta::FromMicroseconds(rtt_us)));
#else
                            rtt_us));
#endif
    ...
    if (IsUtilityAvailable(interval, event_time)) {
      ...
      ++num_available_intervals_;
#ifdef QUIC_PORT
      DLOG_IF(FATAL,num_available_intervals_ > num_useful_intervals_);
#endif
    }
  }
