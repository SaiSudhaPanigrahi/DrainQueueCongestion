...
using namespace ns3;
namespace dqc{
NS_LOG_COMPONENT_DEFINE("queue_limit");
namespace {
...
const size_t kGainCycleLength = 8;//sizeof(kPacingGain) / sizeof(kPacingGain[0]);
...
QueueLimitSender::DebugState::DebugState(const DebugState& state) = default;
QueueLimitSender::QueueMonitor::QueueMonitor():que_th_(kMaxQueueBacklog)
,first_above_ts_(ProtoTime::Zero()){}
void QueueLimitSender::QueueMonitor::Reset(){
    first_above_ts_=ProtoTime::Zero();
    count_=0;
    congestion_=false;
}
void QueueLimitSender::QueueMonitor::NewSample(ProtoTime now,TimeDelta sample,
...
QueueLimitSender::~QueueLimitSender() {}
void QueueLimitSender::SetInitialCongestionWindowInPackets(
...
bool QueueLimitSender::InSlowStart() const {
  return mode_ == STARTUP;
}

void QueueLimitSender::OnPacketSent(ProtoTime sent_time,
...
bool QueueLimitSender::CanSend(QuicByteCount bytes_in_flight) {
  return bytes_in_flight < GetCongestionWindow();
}

QuicBandwidth QueueLimitSender::PacingRate(QuicByteCount bytes_in_flight) const {
...
QuicBandwidth QueueLimitSender::BandwidthEstimate() const {
  return max_bandwidth_.GetBest();
}

QuicByteCount QueueLimitSender::GetCongestionWindow() const {
  if (mode_ == PROBE_RTT) {
...
QuicByteCount QueueLimitSender::GetSlowStartThreshold() const {
  return 0;
}

bool QueueLimitSender::InRecovery() const {
  return recovery_state_ != NOT_IN_RECOVERY;
}

bool QueueLimitSender::ShouldSendProbingPacket() const {
  if (pacing_gain_ <= 1) {
    return false;
  }
...
bool QueueLimitSender::IsPipeSufficientlyFull() const {
...
void QueueLimitSender::AdjustNetworkParameters(QuicBandwidth bandwidth,
                                        TimeDelta rtt,
...
void QueueLimitSender::OnCongestionEvent(bool /*rtt_updated*/,
...
CongestionControlType QueueLimitSender::GetCongestionControlType() const {
  return kQueueLimit;
}

TimeDelta QueueLimitSender::GetMinRtt() const {
  return !min_rtt_.IsZero() ? min_rtt_ : rtt_stats_->initial_rtt();
}

QuicByteCount QueueLimitSender::GetTargetCongestionWindow(float gain) const {
...
QuicByteCount QueueLimitSender::ProbeRttCongestionWindow() const {
...
void QueueLimitSender::EnterStartupMode(ProtoTime now) {
  /*if (stats_) {
...
void QueueLimitSender::DiscardLostPackets(const LostPacketVector& lost_packets) {
...
bool QueueLimitSender::UpdateRoundTripCounter(QuicPacketNumber last_acked_packet) {
...
bool QueueLimitSender::UpdateBandwidthAndMinRtt(
    ProtoTime now,
    const AckedPacketVector& acked_packets) {
...
bool QueueLimitSender::ShouldExtendMinRttExpiry() const {
...
void QueueLimitSender::UpdateGainCyclePhase(ProtoTime now,
                                     QuicByteCount prior_in_flight,
                                     bool has_losses) {
  ...
  if (should_advance_gain_cycling) {
    ...
    if((shadow_gain_==1)&&/*kPacingGain[cycle_current_offset_] == 1 &&*/
            bytes_in_flight > GetTargetCongestionWindow(1)){
    	return;
    }else{
    	shadow_gain_=2;
...
void QueueLimitSender::CheckIfFullBandwidthReached() {
...
void QueueLimitSender::MaybeExitStartupOrDrain(ProtoTime now) {
...
void QueueLimitSender::OnExitStartup(ProtoTime now) {
...
void QueueLimitSender::MaybeEnterOrExitProbeRtt(ProtoTime now,
                                         bool is_round_start,
                                         bool min_rtt_expired) {
...
void QueueLimitSender::UpdateRecoveryState(QuicPacketNumber last_acked_packet,
                                    bool has_losses,
                                    bool is_round_start) {
...
QuicByteCount QueueLimitSender::UpdateAckAggregationBytes(
    ProtoTime ack_time,
...
void QueueLimitSender::CalculateCongestionWindow(QuicByteCount bytes_acked,
  ...
  if(mode_==PROBE_BW){
 	if(shadow_gain_==1){
 		target_window =GetTargetCongestionWindow(1.0);
 	}
   }
  if (is_at_full_bandwidth_) {
...
std::string QueueLimitSender::GetDebugState() const {
  std::ostringstream stream;
  stream << ExportDebugState();
  return stream.str();
}

void QueueLimitSender::OnApplicationLimited(QuicByteCount bytes_in_flight) {
...
QueueLimitSender::DebugState QueueLimitSender::ExportDebugState() const {
  return DebugState(*this);
}

static std::string ModeToString(QueueLimitSender::Mode mode) {
  switch (mode) {
    case QueueLimitSender::STARTUP:
      return "STARTUP";
    case QueueLimitSender::DRAIN:
      return "DRAIN";
    case QueueLimitSender::PROBE_BW:
      return "PROBE_BW";
    case QueueLimitSender::PROBE_RTT:
      return "PROBE_RTT";
  }
  return "???";
}

std::ostream& operator<<(std::ostream& os, const QueueLimitSender::Mode& mode) {
  os << ModeToString(mode);
  return os;
}

std::ostream& operator<<(std::ostream& os, const QueueLimitSender::DebugState& state) {
  ...
  if (state.mode == QueueLimitSender::STARTUP) {
