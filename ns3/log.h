...
void RegisterGlobalStream(std::ostream *stream);
void UnRegisterGlobalStream();
std::ostream* GetGlobalStream();
class LogComponent {
public:
  ...
  void RegisterStream(std::ostream *stream){m_stream=stream;}
  void UnRegisterStream(){m_stream=nullptr;}
private:
  int32_t     m_levels;
  char const *m_name;
public:
