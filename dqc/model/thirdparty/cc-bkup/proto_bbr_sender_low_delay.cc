...
void BbrSender::UpdateGainCyclePhase(ProtoTime now,
  ...
  if(kShadowGain[cycle_current_offset_]<1.0){
		should_advance_gain_cycling=false;
...
// probe drain by window, not by pacing_gain, seem not quit work
 /* if (cycle_current_offset_==3 && bytes_in_flight <= GetTargetCongestionWindow(1)) {
    should_advance_gain_cycling = true;
  }*/
