...
class BbrDrainSender : public SendAlgorithmInterface {
 public:
  enum Mode {
 ...
 // Debug state can be exported in order to troubleshoot potential congestion
  // control issues.
  struct DebugState {
    explicit DebugState(const BbrDrainSender& sender);
  ...
  BbrDrainSender(ProtoTime now,
            const RttStats* rtt_stats,
            const UnackedPacketMapInfoInterface* unacked_packets,
            QuicPacketCount initial_tcp_congestion_window,
            QuicPacketCount max_tcp_congestion_window,
            Random* random);
  BbrDrainSender(const BbrDrainSender&) = delete;
  BbrDrainSender& operator=(const BbrDrainSender&) = delete;
  ~BbrDrainSender() override;
 ...
 std::ostream& operator<<(std::ostream& os,
                                             const BbrDrainSender::Mode& mode);
 std::ostream& operator<<(
    std::ostream& os,
    const BbrDrainSender::DebugState& state);
}
