...
class AimdBbrSender : public SendAlgorithmInterface {
 public:
  enum Mode {
 ...
 // Debug state can be exported in order to troubleshoot potential congestion
  // control issues.
  struct DebugState {
    explicit DebugState(const AimdBbrSender& sender);
  ...
  AimdBbrSender(const ProtoClock* clock,
            ...
            Random* random);
  AimdBbrSender(const AimdBbrSender&) = delete;
  AimdBbrSender& operator=(const AimdBbrSender&) = delete;
  ~AimdBbrSender() override;
 ...
 private:
  ...
  int probe_rtt_counter_{0};
  bool probe_rtt_minimal_cwnd_{false};
};

 std::ostream& operator<<(std::ostream& os,
                                             const AimdBbrSender::Mode& mode);
 std::ostream& operator<<(
    std::ostream& os,
    const AimdBbrSender::DebugState& state);
}
