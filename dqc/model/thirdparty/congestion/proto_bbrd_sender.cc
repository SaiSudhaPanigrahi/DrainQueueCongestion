...
BbrDrainSender::DebugState::DebugState(const BbrDrainSender& sender)
    : mode(sender.mode_),
...
BbrDrainSender::DebugState::DebugState(const DebugState& state) = default;

BbrDrainSender::BbrDrainSender(ProtoTime now,
...
BbrDrainSender::~BbrDrainSender() {}
void BbrDrainSender::SetInitialCongestionWindowInPackets(
...
bool BbrDrainSender::InSlowStart() const {
  return mode_ == STARTUP;
}

void BbrDrainSender::OnPacketSent(ProtoTime sent_time,
...
bool BbrDrainSender::CanSend(QuicByteCount bytes_in_flight) {
  return bytes_in_flight < GetCongestionWindow();
}

QuicBandwidth BbrDrainSender::PacingRate(QuicByteCount bytes_in_flight) const {
...
QuicBandwidth BbrDrainSender::BandwidthEstimate() const {
  return max_bandwidth_.GetBest();
}

QuicByteCount BbrDrainSender::GetCongestionWindow() const {
  if (mode_ == PROBE_RTT) {
...
QuicByteCount BbrDrainSender::GetSlowStartThreshold() const {
  return 0;
}

bool BbrDrainSender::InRecovery() const {
  return recovery_state_ != NOT_IN_RECOVERY;
}

bool BbrDrainSender::ShouldSendProbingPacket() const {
  if (pacing_gain_ <= 1) {
    return false;
  }
...
bool BbrDrainSender::IsPipeSufficientlyFull() const {
...
void BbrDrainSender::AdjustNetworkParameters(QuicBandwidth bandwidth,
                                        TimeDelta rtt,
...
void BbrDrainSender::OnCongestionEvent(bool /*rtt_updated*/,
...
CongestionControlType BbrDrainSender::GetCongestionControlType() const {
  return kBBRD;
}

TimeDelta BbrDrainSender::GetMinRtt() const {
  return !min_rtt_.IsZero() ? min_rtt_ : rtt_stats_->initial_rtt();
}

QuicByteCount BbrDrainSender::GetTargetCongestionWindow(float gain) const {
...
QuicByteCount BbrDrainSender::ProbeRttCongestionWindow() const {
...
void BbrDrainSender::EnterStartupMode(ProtoTime now) {
  mode_ = STARTUP;
  pacing_gain_ = high_gain_;
  congestion_window_gain_ = high_cwnd_gain_;
}

void BbrDrainSender::EnterProbeBandwidthMode(ProtoTime now) {
  mode_ = PROBE_BW;
...
void BbrDrainSender::DiscardLostPackets(const LostPacketVector& lost_packets) {
...
bool BbrDrainSender::UpdateRoundTripCounter(QuicPacketNumber last_acked_packet) {
...
bool BbrDrainSender::UpdateBandwidthAndMinRtt(
    ProtoTime now,
    const AckedPacketVector& acked_packets) {
...
bool BbrDrainSender::ShouldExtendMinRttExpiry() const {
...
void BbrDrainSender::UpdateGainCyclePhase(ProtoTime now,
                                     QuicByteCount prior_in_flight,
                                     bool has_losses) {
...
void BbrDrainSender::CheckIfFullBandwidthReached() {
...
void BbrDrainSender::MaybeExitStartupOrDrain(ProtoTime now) {
...
void BbrDrainSender::OnExitStartup(ProtoTime now) {
...
void BbrDrainSender::MaybeEnterOrExitProbeRtt(ProtoTime now,
                                         bool is_round_start,
                                         bool min_rtt_expired) {
...
void BbrDrainSender::UpdateRecoveryState(QuicPacketNumber last_acked_packet,
                                    bool has_losses,
                                    bool is_round_start) {
...
QuicByteCount BbrDrainSender::UpdateAckAggregationBytes(
    ProtoTime ack_time,
...
void BbrDrainSender::CalculatePacingRate() {
...
void BbrDrainSender::CalculateCongestionWindow(QuicByteCount bytes_acked,
...
void BbrDrainSender::CalculateRecoveryWindow(QuicByteCount bytes_acked,
...
std::string BbrDrainSender::GetDebugState() const {
  std::ostringstream stream;
  stream << ExportDebugState();
  return stream.str();
}

void BbrDrainSender::OnApplicationLimited(QuicByteCount bytes_in_flight) {
...
BbrDrainSender::DebugState BbrDrainSender::ExportDebugState() const {
  return DebugState(*this);
}

static std::string ModeToString(BbrDrainSender::Mode mode) {
  switch (mode) {
    case BbrDrainSender::STARTUP:
      return "STARTUP";
    case BbrDrainSender::DRAIN:
      return "DRAIN";
    case BbrDrainSender::PROBE_BW:
      return "PROBE_BW";
    case BbrDrainSender::PROBE_RTT:
      return "PROBE_RTT";
  }
  return "???";
}

std::ostream& operator<<(std::ostream& os, const BbrDrainSender::Mode& mode) {
  os << ModeToString(mode);
  return os;
}

std::ostream& operator<<(std::ostream& os, const BbrDrainSender::DebugState& state) {
  ...
  if (state.mode == BbrDrainSender::STARTUP) {
